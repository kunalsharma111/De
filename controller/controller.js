...
function findPatData(id) {
    return new Promise((resolve, reject) => {
        MasterPatientModel.find({ _id: id.patientId }, (err, val) => {
            // console.log(val[0]);
            if (!err) {
...
router.post('/postreport', verifyToken, (req, res) => {
    console.log(req.body);
    PostModel.find({ providerName: req.body.provider, visitDate: { "$eq": new Date(req.body.date) } }).then(rese => {
        let returnarray = []
        if (rese.length) {

            rese.forEach(async (id, index) => {
                // console.log(id)
                try {
                    let obj = await findPatData(id);
                    // console.log(obj)
                    returnarray.push(obj);
                    if (index == rese.length - 1) {
